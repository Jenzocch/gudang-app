...
| `MIGRATION_SEED_DIN_MATERIALS.sql` | 一次性資料建檔：DENIKIN 倉包裝材料 12 項＋食品原料 9 項（含分類標籤），初始庫存 0、最低警戒線 10，待現場實際收貨/盤點填入真實數量 | 🆕 2026-07 新增 — 需在 Supabase SQL Editor 執行一次（純資料 seed，可安全重複執行），**須排在 `MIGRATION_ITEMS_REMARK.sql` 之後** |
