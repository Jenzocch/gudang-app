...
## 0.1 四線現況（2026-07-08 更新：①③已通/已建、②待FQMS、④待FQMS對接）

| 線 | 方向與目的 | 接收端程式碼 | 密鑰（header/env） | 狀態 |
|---|---|---|---|---|
| **①** 叫料推送 | FAMMS→Gudang，缺料開單 | Gudang `famms-request` | `x-famms-secret` / `FAMMS_WEBHOOK_SECRET`⇄`GUDANG_WEBHOOK_SECRET` | ✅ **已通（401排查：需 `--no-verify-jwt` 部署＋兩端密鑰值一致）** |
| **②** QC狀態回寫 | FQMS→Gudang，批次變 Hold/Pass | Gudang `qc-lookup`+`qc-status` | `x-qc-secret` / `QC_WEBHOOK_SECRET`⇄`GUDANG_QC_SECRET` | Gudang✅main+部署（`--no-verify-jwt`）；FQMS端待做 |
| **③** 叫料狀態回寫 | Gudang→FAMMS，回報到貨/拒絕 | Gudang `notify-famms-status` → FAMMS `POST /api/external/parts-requests` | `Bearer` / `GUDANG_SYNC_SECRET` | ✅ **兩端都建好**（FAMMS PR#5+#9 已合併 main；Gudang 觸發端本次新增） |
| **④** machine-status | FQMS→FAMMS，查機器能不能跑 | FAMMS `GET /api/external/machine-status` | `Bearer` / `QC_API_SECRET` | FAMMS✅main；FQMS呼叫端待做 |

**線③的設計修正**：FAMMS 那邊最終拿掉了 `qc_result`（parts_requests 只追蹤到貨狀態，不掛 QC 判定）——理由：零件/物料是設備維修用的東西，不是 FQMS 要驗的食品原料，QC 判定本來就該留在原料批次上（線②），不該混進叫料單。所以線③只傳 `status`，不傳 `qc_result`。

## 0.2 ③④ 接收端的實際契約

**④ machine-status（FQMS 呼叫，待建）**
```
...
回應: { machine_code, machine_name, factory_code, status,
        ...
        health_score, open_incident_count, ok_to_run }
錯誤: 401 密鑰 / 400 缺參數 / 404 找不到廠或機台
```

**③ parts-requests 寫回（Gudang `notify-famms-status` 觸發，已建）**
```
Gudang 前端 sb.functions.invoke('notify-famms-status', {body:{request_id}})
  → Gudang Edge Function 查該筆 requests，若 source='famms' 且狀態是終態
    (approved→received / rejected→rejected)：
  → POST https://<famms網域>/api/external/parts-requests
    headers: { Authorization: "Bearer <GUDANG_SYNC_SECRET>" }
    body: { request_id: <famms_request_id>, status: 'received'|'rejected', external_ref: <gudang requests.id> }
回應: { ok:true, request:{...} }  錯誤: 401 / 400 / 404
```

## 0.3 已完成：PR #5 收斂（FAMMS setup session）

`claude/famms-setup-oyy62h`（PR #5，71 commit）已合併進 main，並依照收斂指示處理完叫料重複：
- 送出路徑統一為 main 的 `GudangRequest`（線①）
- `parts_requests` 表降級為唯讀追蹤記錄，由 `/api/gudang/request` 送出成功時自動寫入（存回自己的 id 當 `famms_request_id` 送給 Gudang）
- 舊的手動輸入面板 `PartsRequestPanel` 已刪除
- `external/parts-requests`（線③接收）與 `external/machine-status`（線④）都在 main 上

## 0.4 已完成：Gudang 端線③觸發（本次新增）

1. `migrations/MIGRATION_FAMMS_LINK.sql`：`requests` 表加 `source`、`famms_request_id` 兩欄
2. `famms-request`：接受 body 多一個 `famms_request_id`，連同 `source='famms'` 一併存入
3. 新 Edge Function `notify-famms-status`：讀該筆 requests，若 `source='famms'` 且狀態變終態 → POST FAMMS `external/parts-requests`（best-effort，失敗不影響 Gudang 本地狀態）
4. `index.html` 的 `updReq()`（拒絕）與 `confirmApproveReqBuy()`（已購買入庫）狀態變動後呼叫 `notifyFammsIfNeeded(id)`

**部署待辦（人工）：**
```
supabase secrets set FAMMS_SYNC_URL="https://<famms正式網域>/api/external/parts-requests"
supabase secrets set GUDANG_SYNC_SECRET="<與FAMMS Vercel端GUDANG_SYNC_SECRET同一串>"
supabase functions deploy notify-famms-status
```
以及在 Supabase SQL Editor 執行一次 `migrations/MIGRATION_FAMMS_LINK.sql`。
...
| ③ 寫回 | Gudang→FAMMS | `Bearer` | Gudang `GUDANG_SYNC_SECRET` | FAMMS `GUDANG_SYNC_SECRET`（同一串） |
| ④ 機況 | FQMS→FAMMS | `Bearer` | — | FAMMS `QC_API_SECRET`（FQMS 也持同串） |

> ⚠️ 部署備忘：所有給外部系統呼叫（非 Supabase 前端 SDK）的 Edge Function 都要加 `--no-verify-jwt` 部署，否則 Supabase 閘道會在密鑰比對前就擋掉回 401（線①卡最久的原因）。`notify-famms-status` 例外——它是 Gudang 前端用 `sb.functions.invoke()` 呼叫，帶的是 Supabase anon key，走預設 JWT 驗證即可，不用加這個旗標。
