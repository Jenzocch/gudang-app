...
| `qc-checks-lookup` | FQMS 的 QC 檢查紀錄查詢入口（唯讀）：查某批或某天的 `qc_checks`（含 inspector_name 檢查人），讓兩邊分工不重複檢查 |
...
- **FQMS 品管串接**：FQMS 經 `qc-lookup` 帶入批號、經 `qc-status` 回寫檢驗結果、
  ...
  經 `qc-checks-lookup` 唯讀取得倉庫端 QC 檢查紀錄（含檢查人姓名，讓 FQMS 檢驗員
  知道哪些批倉庫已檢過，兩邊分工不重複輸入）。
